...
namespace OE.ALGA.Tesztek
{
    ...
    public class LancoltListaTesztek
    {
        ...
        public void Bejaras() //F4.
        {
            ...
            ulong bigNumber = (ulong)(new Random().NextDouble() * ulong.MaxValue);
            ulong copy = bigNumber;
            while (copy != 0)
            {
                l.Hozzafuz((int)(copy % 10));
                copy /= 10;
            }

            string s = "";
            l.Bejar((int adat) => s = adat.ToString() + s);
            Assert.AreEqual(s, bigNumber.ToString());
        }
        [TestMethod()]
        [ExpectedException(typeof(HibasIndexKivetel))]
        public void HibasIndexKiolvas()
        {
            Lista<int> l = new LancoltLista<int>();
            l.Kiolvas(0);
        }
        ...
        public void ForeachBejaras() //F5.
        {
            ...
            l.Hozzafuz("d");
            l.Hozzafuz("b");
            LancoltLista<string> k = new LancoltLista<string>();
            string osszefuzo = "";
            foreach (string x in l)
            {
                osszefuzo += x;
            }
            string uresosszefuzo = "";
